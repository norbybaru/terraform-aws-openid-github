locals {
  ...
  merge_conditions = [
    ...
    for k, v in { for c in local.conditions : "${c.test}|${c.variable}" => c... } :
    {
      ...
      # The filter ensures we only merge conditions with matching variables (defensive check)
      "values" : flatten([for index, sp in v[*].values : v[index].values if v[index].variable == v[0].variable])
    }
  ]
}